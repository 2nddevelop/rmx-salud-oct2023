// controllers/NoticiaController.ts
...
const NoticiaController = {
  ...
  createNoticia: async (req: Request, res: Response) => {
    const { nt_titulo, nt_contenido, nt_registrado, nt_usr_id} = req.body;

    try {

      // Crear un nuevo aviso en la base de datos
      const newNoticia = await pool.query('INSERT INTO base_noticias (nt_titulo, nt_contenido, nt_registrado, nt_usr_id) VALUES ($1, $2, $3, $4) RETURNING *', [nt_titulo, nt_contenido, nt_registrado, nt_usr_id]);
