...
const avisosService = {
  ...
  async saveData(newRecord) {
    try {
      const tk = localStorage.getItem('token');
      this.setAuthHeader(tk);
      const response = await axios.post(API_URL + '/aviso', newRecord);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  async updateData(record) {
    try {
      const tk = localStorage.getItem('token');
      this.setAuthHeader(tk);
      const response = await axios.put(API_URL + `/aviso/${record.avi_id}`, record);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  async deleteData(record) {
    try {
      ...
      const tk = localStorage.getItem('token');
      this.setAuthHeader(tk);
      const response = await axios.post(API_URL + `/aviso/${record.avi_id}`, record);
      return response.data;
    } catch (error) {
      throw error;
    }
