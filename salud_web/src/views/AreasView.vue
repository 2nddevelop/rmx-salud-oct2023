  ...
  <div class="">
    ...
    <div style="overflow-x: auto">
      <table class="table table-responsive">
        ...
        <tbody>
          <tr v-for="(r, index) in regs" v-bind:key="r.ar_id">
            ...
            <td align="right">{{ r.ar_descripcion }}</td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
...
export default {
  ...
  methods: {
    ...
    async listarRegistros() {
      try {
        this.regs = await areasService.getData();
    ...
    async saveModal() {
      console.log("Reg: ", this.reg);
      this.reg.ar_usr_id = 1; // <----- Avito completar
      this.reg.ar_estado = "A";
      if (this.isEditing) {
        const updatedReg = await areasService.updateData(this.reg);
        this.regs[
          this.regs.findIndex((reg) => reg.ar_id === updatedReg.ar_id)
        ] = updatedReg;
      } else {
        const savedReg = await areasService.saveData(this.reg);
    ...
    async deleteRegistro(reg) {
      ...
      const indexL = this.regs.findIndex((regX) => reg.ar_id === regX.ar_id);
      console.log("Index: ", indexL);
      console.log("Data: ", this.regs[indexL]);

      this.reg.ar_usr_id = 1; // <----- Avito completar
      this.reg.ar_estado = "X";
      if (window.confirm("¿Estás seguro de eliminar este registro?")) {
        const savedReg = await areasService.deleteData(this.reg);
