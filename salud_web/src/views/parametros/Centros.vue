    ...
    export default {
      ...
      methods: {
          ...
          async saveModal() {
            ...
            this.reg.cnt_usr_id = 1; // <----- Avito completar
          ...
          async deleteRegistro(reg) {
            ...
            const indexL = this.regs.findIndex((regX) => reg.cnt_id === regX.cnt_id);
            console.log("Index: ", indexL);
            console.log("Data: ", this.regs[indexL]);

            this.reg.cnt_usr_id = 1; // <----- Avito completar
            this.reg.cnt_estado = "X";
            if (window.confirm("¿Estás seguro de eliminar este registro?")) {
              const savedReg = await centrosService.deleteData(this.reg);
