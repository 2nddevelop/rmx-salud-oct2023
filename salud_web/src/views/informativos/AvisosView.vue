  ...
  <div class="">
    ...
    <div style="overflow-x: auto">
      <table class="table table-responsive">
        ...
        <tbody>
          <tr v-for="(r, index) in regs" :key="r.avi_id">
            ...
            <td align="right">{{ r.avi_titulo }}</td>
            <td align="right">{{ r.avi_contenido }}</td>
            <td align="right">{{ r.avi_registrado }}</td>
            <td align="right">{{ r.avi_estado }}</td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
          ...
          <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 m-0">
              <div>
                ...
                <input v-model="reg.avi_titulo" class="form-control" name="codigo" id="codigo" placeholder="Titulo" />
              </div>
              <div>
                <label for="descripcion">Contenido:</label>
                <textarea v-model="reg.avi_contenido" class="form-control" name="descripcion" id="descripcion" placeholder="Contenido" rows="5" ></textarea>
              </div>
            </div>
          </div>
          <!-- Modal footer -->
  ...
  export default {
    ...
    methods: {
          async listarRegistros() {
            try {
            this.regs = await avisosService.getData();
        ...
        async saveModal() {
              this.reg.avi_usr_id = 1; 
              this.reg.avi_estado = "A";
            if (this.isEditing) {
              const updatedReg = await avisosService.updateData(this.reg);
              const index = this.regs.findIndex(item => item.avi_id === updatedReg.avi_id);
              if (index !== -1) {
                this.regs.splice(index, 1, updatedReg);
              }
            } else {
              const savedReg = await avisosService.saveData(this.reg);
        ...
        async deleteRegistro(reg) {
          ...
          if (confirmed) {
            try {
              const index = this.regs.findIndex(item => item.avi_id === reg.avi_id);
              if (index !== -1) {
                reg.avi_usr_id = 1;
                reg.avi_estado = "X"; 
                await avisosService.deleteData(reg); 
