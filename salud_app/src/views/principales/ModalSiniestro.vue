<template>
  <ion-page style="height: 700px">
    <ion-header>
      <ion-toolbar>
        <ion-title>Detalles Siniestro</ion-title>
      ...
      <ion-list>
        <ion-item>
          <ion-label class="ion-text-wrap">
            <p><strong>Poliza: {{ item.poliza }}</strong></p>
            <p>{{ item.cliente_ref }}</p>
            <p>{{ item.descripcion }}</p>
            <p>{{ item.descripcion_sin }}</p>
...
console.log('item siniestro >>> ', props.item);
