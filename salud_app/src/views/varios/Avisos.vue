<template>
  <ion-page>
    ...
    <ion-content class="ion-padding">
      <ion-list>
        <ion-item v-for="item in items" :key="item.avi_id">
          <ion-label class="ion-text-wrap">
            <h2>{{ item.avi_titulo }}</h2>
            <p>{{ item.avi_contenido }}</p>
            <p>Estado:&nbsp;
              <ng-template v-if=" item.avi_estado == 'P'" >
                <ion-badge color="warning">Pendiente</ion-badge>
              </ng-template>
              <ng-template v-if=" item.avi_estado == 'A'" >
...
import { useServiceAvisos } from '@/services/serviceAvisos';

let items = ref([]);

const apiService = useServiceAvisos();
