<template>
  <ion-page>
    ...
    <ion-content class="ion-padding">
      <ion-list>
        <ion-item v-for="item in items" :key="item.hor_id">
          <ion-label class="ion-text-wrap">
            <h2>{{ item.hor_codigo }}</h2>
            <p>{{ item.hor_descripcion }}</p>
            <p>De: {{ item.hor_de }} A: {{ item.hor_a }}</p>
            <p>Estado:&nbsp;
              <ng-template v-if=" item.hor_estado == 'P'" >
                <ion-badge color="warning">Pendiente</ion-badge>
              </ng-template>
              <ng-template v-if=" item.hor_estado == 'A'" >
...
import { useServiceHorarios } from '@/services/serviceHorarios';

let items = ref([]);

const apiService = useServiceHorarios();
